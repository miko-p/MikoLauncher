...
| **插件化 UI 骨架（M9-6）** | `UiViewSchema`（key/label/path/order/builtin/type/html/disabled/**actions**）并入 `UiManifestSchema.views`；sidecar `UiRegistryService` 种子化内置五视图 + `registerView()`（插件可增/删导航项与页面，卸载 effect 移除）；前端 `App.vue` 导航条改由 `uiStore.views` 渲染 + `router.addRoute` 动态注册插件视图 → `PluginHtmlView`；示例插件 `demo-view` 贡献导航页 + **可点动作**（`actions` 声明 + `view.<key>.<action>` handler，经 Rust `plugin_view_action` 触发回显） | sidecar `ui.getManifest` 返回 6 views（5 内置 builtin + 1 插件 demo-view 含 actions）✓；`view.demo-view.greet/toggle` 往返 ✓；未知动作拒绝 ✓；`pnpm build` 全绿；cargo check/clippy 零告警；`--self-check` 全绿 |
...
- **M9-6 插件化 UI 骨架**：把顶栏导航 + 页面路由从「前端硬编码五页」升级为「由 ui manifest 驱动」——sidecar `UiRegistryService` 在构造时种子化内置五视图（builtin=true），插件经 `registerView()` 可**新增/覆盖**导航项与页面（key 幂等），卸载 `effect` 自动移除（builtin 视图保留）。前端 `App.vue` 导航条改渲染 `uiStore.views`（过滤 disabled + 按 order 排序），`router.addRoute` 动态注册插件视图 → `PluginHtmlView`（v-html 渲染插件 html 内容）。
  - **交互承载边界**：插件视图内容 `type='html'`（v-html，与布局 slot 同通道，宿主信任 phase0 hash + CSP 限内联脚本），**可点交互通过 `actions` 声明达成**：插件视图贡献 `actions:[{id,label}]` 并在 sidecar `ctx.rustBridge.on('view.<key>.<action>', handler)` 注册处理器；前端渲染动作按钮，点击经 Rust `plugin_view_action` → `view.<key>.<action>` → handler，结果回显页面。方法名由 key+action 动态拼出、限定在 `view.*` 命名空间（未知动作 sidecar 返回 METHOD_NOT_FOUND），安全且插件代码留在 sidecar 侧。更rich 的组件级交互（Vue 组件、沙箱）仍留待分发演进 Phase 2。
  - **dev 误走打包分支修复**：`resolve_plugin_host()` 用 `cfg!(debug_assertions)` 排除——dev（debug build）恒走 tsx 源码分支扫 repo `plugins/`，不受 target/debug 残留打包版 sidecar（build-binary/tauri build 产物）污染（否则注入 release env、扫 `~/.local/share`、插件不装载导航空白）。
...
对 sidecar 直接调 `view.demo-view.greet` / `view.demo-view.toggle`（demo-view 声明的 actions handler）：
```
=== demo-view 视图 ===
 label: 示例视图 | actions: [{"id":"greet","label":"打个招呼"},{"id":"toggle","label":"切换状态"}]
=== 动作 [greet] ===  {"title":"来自插件 handler","message":"你好！..."}
=== 动作 [toggle] === {"title":"状态切换","message":"当前状态：关"}
未知动作 view.demo-view.nope → 正确拒绝 METHOD_NOT_FOUND
```
- 前端 `PluginHtmlView` 渲染 actions 按钮，点击经 Rust `plugin_view_action` → `view.<key>.<action>` → handler → 结果回显；未知动作拒绝（安全）。
...
- **M9-6 修复（dev 误走打包分支）**：`apps/desktop/src-tauri/src/lib.rs` `resolve_plugin_host()` 加 `cfg!(debug_assertions)` 排除——dev（debug build）恒走 tsx 源码扫 repo `plugins/`，不受 target/debug 残留打包版 sidecar 污染（否则注入 release env、插件不装载）
...
- **M9-6**：`packages/shared/src/entities.ts`（`UiViewSchema` + `UiManifestSchema.views` + `UiViewActionSchema`）、`packages/shared/src/methods.ts`（`viewAction` 契约）、`apps/plugin-host/src/services/ui-registry.ts`（内置视图种子化 + `registerView`）、`apps/desktop/src-tauri/src/lib.rs`（`plugin_view_action` command）、`apps/desktop/src/api/index.ts`（`viewAction`）、`apps/desktop/src/stores/ui.ts`（`views`/`pluginViews`）、`apps/desktop/src/router/index.ts`（`syncPluginRoutes` 动态 addRoute）、`apps/desktop/src/views/PluginHtmlView.vue`（插件页面渲染 + actions 按钮/结果回显）、`apps/desktop/src/App.vue`（导航改由 manifest 渲染）、`plugins/demo-view/`（示例视图插件 + 交互动作）
