...
### M9 已添加
- **插件化 UI 骨架（M9-6）**：把顶栏导航 + 页面路由从「前端硬编码五页」升级为由 ui manifest 驱动 —— `UiViewSchema`（key/label/path/order/builtin/type/html/disabled）并入 `UiManifestSchema.views`；sidecar `UiRegistryService` 种子化内置五视图 + `registerView()`（插件可**新增/覆盖**导航项与页面，卸载 `effect` 自动移除 builtin 外的视图）；前端 `App.vue` 导航条改由 `uiStore.views` 渲染（过滤 disabled + 按 order 排序）+ `router.addRoute` 动态注册插件视图 → `PluginHtmlView`（v-html 渲染插件内容，与布局 slot 同通道）。示例插件 `demo-view` 贡献一个「示例视图」导航页。交互承载边界：插件视图当前 `type='html'`（静态/装饰），组件级交互（Vue 组件、可执行逻辑、调 Rust）留待分发演进 Phase 2。
